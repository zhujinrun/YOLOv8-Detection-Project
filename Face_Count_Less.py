...
out = cv2.VideoWriter('result/students_count_less.mp4', fourcc, fps, (width, height))

# 计算每隔 n 秒的帧数间隔
frame_interval = fps * 5  # n 秒间隔
frame_count = 0
        ...
        if start_frame <= frame_count <= end_frame:
            ...
            if frame_count % frame_interval == 1:
                ...
                if len(results) > 0 and hasattr(results[0].boxes, 'xyxy'):
                    ...
                    # 记录时间节点和抬头数量到CSV文件
                    csv_file = 'result/students_count_less.csv'
                ...
                # 显示时间节点和抬头数量
                cv2.putText(frame, f'Raise Head Count: {face_count}', (10, 30),
